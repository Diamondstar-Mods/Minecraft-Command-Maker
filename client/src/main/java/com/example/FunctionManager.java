...
public class FunctionManager {
    ...
    public static void executeFunction(String functionName, MinecraftClient client) {
        try {
            Path functionFile = FUNCTIONS_PATH.resolve(functionName + ".mcfunction");
            if (!Files.exists(functionFile)) {
                if (client.player != null) {
                    client.player.sendMessage(Text.literal("§c✖ Function §f" + functionName + "§c not found"), false);
                }
                return;
            }
            ...
            for (int idx = 0; idx < lines.size(); idx++) {
                ...
                try {
                    ...
                    client.player.sendMessage(
                        Text.literal("§c✖ Error in §f" + functionName + "§c at line §f" + lineNum + "§c: §7" + ex.getMessage()),
                        false);
                    return;
                }
            }
            ...
            if (client.player != null) {
                client.player.sendMessage(Text.literal("§c✖ Failed to run §f" + functionName + "§c: §7" + e.getMessage()), false);
            }
        }
    }

    public static void downloadFunction(String functionName, MinecraftClient client) {
        new Thread(() -> {
            try {
                ...
                if (response.statusCode() == 200) {
                    ...
                    if (client.player != null) {
                        client.player.sendMessage(
                            Text.literal("§a✔ Downloaded §f" + functionName + "§a successfully! §7Edit: config/CommandMaker/Functions/" + functionName + ".mcfunction"),
                            false);
                    }
                } else {
                    if (client.player != null) {
                        client.player.sendMessage(
                            Text.literal("§c✖ Download failed for §f" + functionName + "§c: HTTP §7" + response.statusCode()),
                            false);
                    }
                }
            } catch (Exception e) {
                if (client.player != null) {
                    client.player.sendMessage(Text.literal("§c✖ Download failed: §7" + e.getMessage()), false);
                }
            }
        }).start();
    }
