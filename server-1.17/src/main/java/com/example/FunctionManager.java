...
public class FunctionManager {
    ...
    public static int executeFunction(String functionName, CommandContext<ServerCommandSource> ctx) {
        try {
            ...
            if (!Files.exists(functionFile)) {
                ctx.getSource().sendFeedback(new LiteralText("§c✖ Function §f" + functionName + "§c not found"), false);
                return 0;
            }
            ...
            for (int idx = 0; idx < lines.size(); idx++) {
                ...
                try {
                    ...
                    ctx.getSource().sendFeedback(new LiteralText("§c✖ Error in §f" + functionName + "§c at line §f" + lineNum + "§c: §7" + errorMsg), false);
                    return executed;
                }
            }
            return executed;
            ...
            ctx.getSource().sendFeedback(new LiteralText("§c✖ Failed to run §f" + functionName + "§c: §7" + e.getMessage()), false);
            return 0;
        }
    }
        ...
        new Thread(() -> {
            try {
                ...
                if (response.statusCode() == 200) {
                    ...
                    source.sendFeedback(new LiteralText("§a✔ Downloaded §f" + functionName + "§a successfully! §7Edit: config/CommandMaker/Functions/" + functionName + ".mcfunction"), false);
                } else {
                    source.sendFeedback(new LiteralText("§c✖ Download failed for §f" + functionName + "§c: HTTP §7" + response.statusCode()), false);
                }
            } catch (Exception e) {
                source.sendFeedback(new LiteralText("§c✖ Download failed: §7" + e.getMessage()), false);
            }
        }).start();
    }
    ...
    public static void listDownloadableFunctions(ServerCommandSource source) {
        source.sendFeedback(new LiteralText("§6⌛ Fetching function library..."), false);
        new Thread(() -> {
            try {
                Map<String, String> manifest = fetchFunctionManifest();
                if (manifest.isEmpty()) {
                    source.sendFeedback(new LiteralText("§c✖ No downloadable functions available"), false);
                } else {
                    source.sendFeedback(new LiteralText("§6§l▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"), false);
                    source.sendFeedback(new LiteralText("§6§l📦 Downloadable Functions §7(§f" + manifest.size() + "§7 available)"), false);
                    source.sendFeedback(new LiteralText("§7Use §e/cmd downloadfunction <name> §7or open the GUI with §e/cmd functions"), false);
                    source.sendFeedback(new LiteralText("§6§l▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬"), false);
                    for (Map.Entry<String, String> entry : manifest.entrySet()) {
                        String name = entry.getKey();
                        String desc = entry.getValue();
                        if (desc != null && !desc.isEmpty()) {
                            source.sendFeedback(new LiteralText("§a  ◆ §f" + name + " §8▶ §7" + desc), false);
                        } else {
                            source.sendFeedback(new LiteralText("§a  ◆ §f" + name), false);
                        }
                    }
                    source.sendFeedback(new LiteralText("§7─────────────────────────────"), false);
                }
            } catch (Exception e) {
                source.sendFeedback(new LiteralText("§cFailed to fetch function list: " + e.getMessage()), false);
            }
        }).start();
    }
