...
public class ModuleManager {
     ...
     */
    public static boolean exportModule(String moduleName, boolean includeFunctions, ServerCommandSource source) {
        try {
            ...
            try (ZipOutputStream zos = new ZipOutputStream(new FileOutputStream(outputFile.toFile()))) {
                ...
                meta.addProperty("author", source.getName());
            ...
            source.sendFeedback(() -> Text.literal("§a✔ Module exported: §f" + moduleName + ".cmk §7| " + outputFile), false);
            LOGGER.info("Module exported: {}", outputFile);
            return true;
        } catch (Exception e) {
            LOGGER.error("Failed to export module '{}'", moduleName, e);
            source.sendFeedback(() -> Text.literal("§c✖ Export failed: §7" + e.getMessage()), false);
            return false;
        }
    }
     ...
     */
    public static boolean importModule(String moduleName, boolean overwrite, ServerCommandSource source) {
        try {
            Path moduleFile = MODULES_PATH.resolve(moduleName + ".cmk");
            if (!Files.exists(moduleFile)) {
                source.sendFeedback(() -> Text.literal("§c✖ Module file not found: §f" + moduleName + ".cmk"), false);
                return false;
            }
            ...
            for (String item : imported) {
                source.sendFeedback(() -> Text.literal("  §a+ §f" + item), false);
            }
            if (!skipped.isEmpty()) {
                source.sendFeedback(() -> Text.literal("§e⚠ Skipped §f" + skipped.size() + "§e items (use --overwrite to replace):"), false);
                for (String item : skipped) {
                    source.sendFeedback(() -> Text.literal("  §7- §f" + item), false);
                }
            }
            ...
            source.sendFeedback(() -> Text.literal("§c✖ Import failed: §7" + e.getMessage()), false);
            return false;
        }
    }
