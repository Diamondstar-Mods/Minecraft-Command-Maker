								...
								.then(
									net.minecraft.server.command.CommandManager.argument("target", StringArgumentType.greedyString())
										.executes(ctx -> {
											...
											if (onlyOperatorsCanRunAliases && !source.hasPermissionLevel(2)) {
												source.sendFeedback(() -> net.minecraft.text.Text.literal("Only operators can add aliases."), false);
												return 0;
											}
						...
						.then(
							net.minecraft.server.command.CommandManager.argument("alias", StringArgumentType.word())
								.executes(ctx -> {
									...
									// Check if only operators can manage aliases
									if (onlyOperatorsCanRunAliases && !source.hasPermissionLevel(2)) {
										source.sendFeedback(() -> net.minecraft.text.Text.literal("Only operators can delete aliases."), false);
										return 0;
									}
		...
		dispatcher.register(
			net.minecraft.server.command.CommandManager.literal(alias)
				.executes(ctx -> {
					...
					if (onlyOperatorsCanRunAliases && !source.hasPermissionLevel(2)) {
						if (!commandsRunByNonAdminsAllowOverride.contains(alias)) {
							source.sendFeedback(() -> net.minecraft.text.Text.literal("You don't have permission to run this alias."), false);
							return 0;
						}
					}
					
					// Check if this command should run in console mode
					boolean runInConsole = commandsRunInConsoleMode.contains(alias);
					...
					if (target.startsWith("function:")) {
						...
						if (runInConsole) {
							// Execute as console (server)
							ServerCommandSource consoleSource = source.getServer().getCommandSource();
							ParseResults<ServerCommandSource> parsed = cmdDispatcher.parse(command, consoleSource);
