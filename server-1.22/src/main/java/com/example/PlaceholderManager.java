...
public class PlaceholderManager {
    ...
    public static class PlaceholderContext {
        ...
        public PlaceholderContext(ServerPlayer player) {
            this.player = player;
            this.server = player != null ? player.createCommandSourceStack().getServer() : null;
        }

        public PlaceholderContext(ServerPlayer player, MinecraftServer server) {
            this.player = player;
            this.server = server;
        }
    }
    ...
    /**
     * Resolve placeholders using a ServerPlayer directly.
     */
    public static String resolvePlaceholders(String text, net.minecraft.commands.CommandSourceStack source) {
    ...
    private static String resolveSinglePlaceholder(String placeholder, PlaceholderContext ctx) {
        ...
        if (ctx.player != null) {
            switch (key) {
                ...
                case "player_food":
                    return String.valueOf(ctx.player.getFoodData().getFoodLevel());
                case "player_world":
                    return ctx.player.level().dimension().registry().toString();
                case "player_gamemode":
                    return ctx.player.gameMode.getGameModeForPlayer().getName();
                case "player_ping":
                    return "0"; // TODO: fix for MC 26.1 mappings
            }
        ...
        if (ctx.server != null) {
            switch (key) {
                case "server_online":
                    return String.valueOf(ctx.server.getPlayerList().getPlayerCount());
                case "server_max":
                    return String.valueOf(ctx.server.getMaxPlayers());
                case "server_tps":
