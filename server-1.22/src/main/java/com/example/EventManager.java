...
public class EventManager {
     ...
     */
    public static void fireEvent(String eventType, ServerPlayer player, Map<String, Object> context) {
    ...
    private static String substituteEventVariables(String command, ServerPlayer player, Map<String, Object> context) {
        ...
        result = result.replace("${world}", player.level().dimension().registry().toString());
    ...
    private static void registerListeners() {
        ...
        ServerLivingEntityEvents.AFTER_DEATH.register((LivingEntity entity, DamageSource damageSource) -> {
            if (entity instanceof ServerPlayer player) {
                ...
                ctx.put("death_cause", damageSource.type().msgId());
                if (damageSource.getEntity() != null) {
                    ctx.put("killer", damageSource.getEntity().getName().getString());
                }
        ...
        PlayerBlockBreakEvents.AFTER.register((world, player, pos, state, blockEntity) -> {
            if (events.containsKey("block_break")) {
                ...
                if (cfg.blocks.isEmpty() || cfg.blocks.contains(blockId)) {
                    ...
                    fireEvent("block_break", (ServerPlayer) player, ctx);
                }
            }
        });
        ...
        ServerMessageEvents.CHAT_MESSAGE.register((message, sender, params) -> {
            if (events.containsKey("player_chat")) {
                ...
                Map<String, Object> ctx = new HashMap<>();
                ctx.put("message", message.decoratedContent().getString());
