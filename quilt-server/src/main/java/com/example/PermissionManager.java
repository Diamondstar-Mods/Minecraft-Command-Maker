...
public class PermissionManager {
    ...
    public static void initialize() {
        try {
            ...
            initializeLuckPerms();
            CommandMaker.LOGGER.info("Permission Manager initialized successfully");
        } catch (Exception e) {
            CommandMaker.LOGGER.error("Failed to initialize Permission Manager", e);
        }
    }
    ...
    public static boolean hasPermission(ServerCommandSource source, String permissionNode) {
        try {
            ...
            CommandMaker.LOGGER.warn("Error checking permission: {}", permissionNode, e);
            return false;
        }
    }
    ...
    public static boolean canUseAlias(ServerCommandSource source, String aliasName) {
        try {
            ...
            CommandMaker.LOGGER.warn("Error checking alias permission: {}", aliasName, e);
            return false;
        }
    }
    ...
    private static boolean checkConfigPermission(UUID playerUuid, String permissionNode) {
        try {
            ...
            CommandMaker.LOGGER.warn("Error checking config permission", e);
            return false;
        }
    }
    ...
    private static void initializeLuckPerms() {
        try {
            ...
            CommandMaker.LOGGER.info("LuckPerms detected - enabling LuckPerms integration");
            useLuckPerms = true;
        } catch (ClassNotFoundException e) {
            CommandMaker.LOGGER.info("LuckPerms not found - using built-in permissions only");
            useLuckPerms = false;
        } catch (Exception e) {
            CommandMaker.LOGGER.warn("Failed to initialize LuckPerms", e);
            useLuckPerms = false;
        }
    }
    ...
    private static boolean checkLuckPermsPermission(ServerPlayerEntity player, String permissionNode) {
        try {
            ...
            CommandMaker.LOGGER.warn("Error checking LuckPerms permission", e);
            return false;
        }
    }
    ...
    private static void ensurePermissionsConfigExists() {
        try {
            ...
            if (!Files.exists(PERMS_CONFIG)) {
                ...
                CommandMaker.LOGGER.info("Created default permissions config at: {}", PERMS_CONFIG);
            }
        } catch (Exception e) {
            CommandMaker.LOGGER.error("Failed to ensure permissions config exists", e);
        }
    }
    ...
    private static void loadPermissionsConfig() {
        try {
            ...
            CommandMaker.LOGGER.info("Loaded permissions config from: {}", PERMS_CONFIG);
        } catch (Exception e) {
            CommandMaker.LOGGER.error("Failed to load permissions config", e);
    ...
    public static void grantPermission(UUID playerUuid, String permissionNode) {
        try {
            ...
            CommandMaker.LOGGER.info("Granted permission {} to player {}", permissionNode, playerUuid);
        } catch (Exception e) {
            CommandMaker.LOGGER.error("Failed to grant permission", e);
        }
    }
    ...
    public static void revokePermission(UUID playerUuid, String permissionNode) {
        try {
            ...
            CommandMaker.LOGGER.info("Revoked permission {} from player {}", permissionNode, playerUuid);
        } catch (Exception e) {
            CommandMaker.LOGGER.error("Failed to revoke permission", e);
        }
    }
    ...
    private static void savePermissionsConfig() {
        try {
            ...
            CommandMaker.LOGGER.info("Saved permissions config");
        } catch (Exception e) {
            CommandMaker.LOGGER.error("Failed to save permissions config", e);
        }
    }
    ...
    public static PermissionLevel getPermissionLevel(ServerCommandSource source) {
        try {
            ...
            CommandMaker.LOGGER.warn("Error getting permission level", e);
            return PermissionLevel.NONE;
        }
    }
}
